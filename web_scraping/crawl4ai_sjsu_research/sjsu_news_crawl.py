import os
import json
import asyncio
from pydantic import BaseModel, Field
from crawl4ai import AsyncWebCrawler, CrawlerRunConfig, LLMConfig, BrowserConfig, CacheMode
from crawl4ai import LLMExtractionStrategy

class NewsArticle(BaseModel):
    title: str = Field(..., description="Title of the news article.")
    date: str = Field(..., description="Publication date of the news article.")
    url: str = Field(..., description="URL link to the full news article.")

async def extract_sjsu_news(
    provider, api_token=None, extra_headers=None
):
    print(f"\n--- Extracting SJSU News with {provider} ---")

    if api_token is None and provider != "ollama":
        print(f"API token is required for {provider}. Skipping this example.")
        return

    browser_config = BrowserConfig(headless=True)

    extra_args = {"temperature": 0, "top_p": 0.9, "max_tokens": 2000}
    if extra_headers:
        extra_args["extra_headers"] = extra_headers

    crawler_config = CrawlerRunConfig(
        cache_mode=CacheMode.BYPASS,
        word_count_threshold=1,
        page_timeout=80000,
        extraction_strategy=LLMExtractionStrategy(
            llm_config=LLMConfig(provider=provider, api_token=api_token),
            schema=NewsArticle.model_json_schema(),
            extraction_type="schema",
            instruction="""From the links provided, extract the entire news article and clean any hyperlinks or extraneous information. Also extract the title, date, and URL.""",
            extra_args=extra_args,
        ),
    )

    async with AsyncWebCrawler(config=browser_config) as crawler:
        result = await crawler.arun(
            url="https://www.sjsu.edu/news/",
            config=crawler_config
        )
        print(result.extracted_content)

        with open("links.json", "w") as fp:
            json.dump(json.loads(result.extracted_content), fp, indent=4, ensure_ascii=False)

if __name__ == "__main__":
    asyncio.run(
        extract_sjsu_news(
            provider="openai/gpt-4o", api_token=os.getenv("OPENAI_API_KEY")
        )
    )